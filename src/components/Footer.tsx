...
const FooterColumn: React.FC<FooterColumnProps> = ({ title, links, width, delay }) => {
  return (
    <div className={`${width} animate-fade-in`} style={{ animationDelay: `${delay}ms` }}>
      <div className="text-[#9747FF] transition-colors duration-300 hover:text-[#27CDFE]">{title}</div>
      {links.map((link, index) => (
        <button
          key={index}
          className="text-[#BECCE5] mt-4 hover:text-[#27CDFE] transition-all duration-300 hover:scale-105 block text-left"
        >
          {link}
        </button>
      ))}
          ...
          <div className="self-center flex items-center gap-8 overflow-hidden mt-6">
            {socialIcons.map((icon, index) => (
              ...
              >
                <img
                  src={icon}
                  className="aspect-[1] object-contain w-6 self-stretch shrink-0 my-auto animate-fade-in"
